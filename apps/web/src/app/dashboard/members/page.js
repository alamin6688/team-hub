...
import { 
  UserPlus, MoreVertical, Trash2, Shield, 
  User, Mail, Calendar, Search, X, Check, Send,
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-normal text-sm">Manage your team and their roles</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {[
          ...
          { label: 'Admins', value: members.filter(m => m.role === 'ADMIN').length, color: 'text-rose-600', bg: 'bg-rose-50' },
          { label: 'Online Now', value: members.filter(m => m.isOnline).length, color: 'text-emerald-600', bg: 'bg-emerald-50' },
        ].map((stat, i) => (
          <motion.div 
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="text-left bg-slate-50/50">
                <th className="px-8 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Member</th>
                ...
                {isAdmin && <th className="px-8 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-widest text-right">Actions</th>}
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        <div className="relative">
                          {member.avatarUrl ? (
                            <img src={member.avatarUrl} className="w-12 h-12 rounded-2xl object-cover shadow-sm" alt="" />
                          ) : (
                          ...
                          <div className={`absolute -bottom-1 -right-1 w-4 h-4 border-2 border-white rounded-full ${member.isOnline ? 'bg-emerald-500' : 'bg-slate-300'}`}></div>
                        </div>
                        <div>
                          <div className="font-semibold text-slate-900 flex items-center gap-2">
                            {member.name}
                            {member.id === currentUser?.id && (
                              <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full uppercase font-bold tracking-tighter">You</span>
                            )}
                            {member.isBlocked && (
                              <span className="text-[10px] bg-rose-100 text-rose-500 px-2 py-0.5 rounded-full uppercase font-bold tracking-tighter">Blocked</span>
                            )}
                          </div>
                    ...
                    <td className="px-8 py-5">
                      {isAdmin && member.id !== currentUser?.id ? (
                        ...
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-bold uppercase tracking-wider ${
                          member.role === 'ADMIN' ? 'bg-rose-50 text-rose-600' : 'bg-slate-50 text-slate-600'
                        }`}>
                    ...
                    <td className="px-8 py-5">
                      <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-[11px] font-bold ${
                        member.isOnline ? 'text-emerald-600 bg-emerald-50' : 'text-slate-400 bg-slate-50'
                      }`}>
                        <div className={`w-2 h-2 rounded-full ${member.isOnline ? 'bg-emerald-500 shadow-lg shadow-emerald-200' : 'bg-slate-300'}`}></div>
                      ...
                      <td className="px-8 py-5 text-right">
                        {member.id !== currentUser?.id ? (
                          <div className="flex items-center justify-end gap-2">
                            <button 
                              onClick={() => setShowConfirmModal({ type: 'block', member })}
                            ...
                            >
                              {member.isBlocked ? <Unlock size={18} /> : <Lock size={18} />}
                            </button>
                            <button 
                              onClick={() => setShowConfirmModal({ type: 'remove', member })}
                          ...
                          <span className="text-[11px] font-bold text-slate-300 uppercase tracking-widest italic pr-4">Owner</span>
                        )}
                      </td>
                    )}
                  </motion.tr>
          ...
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
            ...
            >
              <div className="mb-10 text-center">
                ...
                <h3 className="text-3xl font-bold text-slate-900">Invite Member</h3>
                <p className="text-slate-500 font-medium mt-2">Send an email invitation to join your team</p>
              </div>
              ...
              <form onSubmit={handleInvite} className="space-y-6">
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-3 ml-1">Assign Role</label>
                ...
                <div className="flex gap-4 pt-6">
                  ...
                  <button 
                    ...
                    className="flex-[2] px-8 py-5 bg-[#f43f5e] text-white font-bold rounded-2xl hover:bg-[#e11d48] shadow-2xl shadow-rose-100 transition-all active:scale-[0.98] disabled:opacity-70 flex items-center justify-center gap-3"
                  >
                    {isSubmitting ? (
          ...
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
            ...
            >
              <div className={`w-16 h-16 rounded-[20px] flex items-center justify-center mx-auto mb-6 ${
                showConfirmModal.type === 'remove' ? 'bg-rose-50 text-rose-500' : 'bg-amber-50 text-amber-500'
              }`}>
                {showConfirmModal.type === 'remove' ? <ShieldAlert size={32} /> : <Lock size={32} />}
              </div>
              
              <h3 className="text-2xl font-bold text-slate-900 mb-2">
                {showConfirmModal.type === 'remove' ? 'Remove Member?' : (showConfirmModal.member.isBlocked ? 'Unblock Member?' : 'Block Member?')}
              </h3>
              ...
              <div className="flex gap-4">
                ...
                <button 
                  onClick={showConfirmModal.type === 'remove' ? handleRemoveMember : handleBlockMember}
                  className={`flex-1 px-6 py-4 text-white font-bold rounded-2xl transition-all active:scale-[0.98] ${
                    showConfirmModal.type === 'remove' ? 'bg-rose-500 hover:bg-rose-600' : 'bg-amber-500 hover:bg-amber-600'
                  }`}
                >
                  Confirm
                </button>
