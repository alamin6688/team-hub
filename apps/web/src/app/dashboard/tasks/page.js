...
export default function TasksPage() {
  ...
  const onDragEnd = async (result) => {
    ...
    try {
      await updateActionItem(currentWorkspace.id, draggableId, {
        status: newStatus,
      });
              ...
              <div className="flex gap-6 items-start overflow-x-auto pb-4">
                {columns.map((column) => (
                  <motion.div
                    key={column.id}
                    variants={itemVariants}
                    className="flex-1 min-w-[320px] bg-gray-50/50 rounded-2xl p-4 border border-gray-100 flex flex-col min-h-[70vh]"
                  >
                    <div className="flex justify-between items-center mb-5 px-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="bg-white border border-gray-200 text-gray-500 text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm">
                          {
                            actionItems.filter((t) => t.status === column.id)
                              .length
                          }
                        </span>
                      </div>
                            ...
                            .map((task, index) => (
                              ...
                              >
                                {(provided, snapshot) => (
                                  ...
                                  >
                                    <div
                                      className={`absolute left-0 top-0 bottom-0 w-1.5 ${priorityConfig[task.priority]?.dot || "bg-gray-400"}`}
                                    ></div>

                                    <div className="flex justify-between items-start mb-4 ml-2">
                                      <div className="flex flex-col gap-1 pr-4">
                                        <span
                                          className={`text-sm font-bold leading-tight ${task.status === "DONE" ? "text-gray-400 line-through" : "text-gray-800"}`}
                                        >
                                          {task.title}
                                        </span>
                                        {task.goal && (
                                      ...
                                      <div className="flex-shrink-0">
                                        {task.assignee ? (
                                          <div className="relative group/avatar">
                                            <div
                                              className={`w-8 h-8 rounded-full bg-indigo-600 text-white flex items-center justify-center text-[10px] font-bold shadow-sm border-2 border-white`}
                                            >
                                              {task.assignee.name
                                                .substring(0, 2)
                                                .toUpperCase()}
                                    ...
                                    <div className="ml-2 flex items-center justify-between">
                                      <div className="flex items-center gap-3">
                                        ...
                                        {task.priority && (
                                          <span
                                            className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${priorityConfig[task.priority].text} bg-white border ${priorityConfig[task.priority].border}`}
                                          >
                                            {task.priority}
                                          </span>
                                        )}
          ...
          >
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-gray-50/50 border-b border-gray-100">
                <tr>
                  <th className="px-8 py-5 font-bold text-gray-400 text-[11px] uppercase tracking-wider w-[40%]">
                    Task Title
                  </th>
                  <th className="px-8 py-5 font-bold text-gray-400 text-[11px] uppercase tracking-wider w-[15%]">
                    Priority
                  </th>
                  <th className="px-8 py-5 font-bold text-gray-400 text-[11px] uppercase tracking-wider w-[15%]">
                    Status
                  ...
                  <th className="px-8 py-5 font-bold text-gray-400 text-[11px] uppercase tracking-wider w-[10%]">
                    Due
                  </th>
                  ...
                  >
                    <td className="px-8 py-5">
                      <div className="flex flex-col gap-1">
                        <span
                          className={`font-bold text-gray-800 ${task.status === "DONE" ? "line-through text-gray-400" : ""}`}
                        >
                          {task.title}
                        </span>
                        {task.goal && (
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-2 h-2 rounded-full ${priorityConfig[task.priority]?.dot || "bg-gray-300"}`}
                        ></div>
                        <span
                          className={`text-xs font-bold ${priorityConfig[task.priority]?.text || "text-gray-500"}`}
                        >
                    ...
                    <td className="px-8 py-5">
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold tracking-tight uppercase ${statusConfig[task.status]?.bg || "bg-gray-100"} ${statusConfig[task.status]?.text || "text-gray-600"}`}
                      >
        ...
        {showAddModal && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative bg-white rounded-3xl shadow-2xl border border-slate-100 p-8 max-w-xl w-full"
            >
              <button
                onClick={() => setShowAddModal(false)}
        ...
        {deleteModal.isOpen && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative bg-white rounded-2xl shadow-2xl border border-slate-100 p-8 max-w-sm w-full text-center"
            >
